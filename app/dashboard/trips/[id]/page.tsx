...
export default function TripDetailPage() {
  ...
  const [selectedMatch, setSelectedMatch] = useState<MatchWithParcel | null>(
    null
  );
  ...
  const loadMatches = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        console.log(
          `[TRIP DETAIL] Loaded ${validMatches.length} matches for trip ${tripId}`
        );
        setMatches(validMatches);
      } else {
        console.error(
          "[TRIP DETAIL] Failed to load matches:",
  ...
  const handleEdit = () => {
    ...
    const destinationParsed = parseAddressString(
                ...
                <dl className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-gray-900">
                        {formatDateTime(trip.estimated_arrival)}
                      </dd>
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-lg shadow p-6">
                ...
                {loadingMatches ? (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            {match.estimated_delivery_fee != null &&
                              match.estimated_currency && (
                                <p className="text-sm font-medium text-green-700 mb-2">
                                  You&apos;ll receive:{" "}
                                  {match.estimated_currency}{" "}
                                  {Number(match.estimated_delivery_fee).toFixed(
                                    2
                                  )}
                                </p>
                              )}
                            {match.parcel?.sender && (
                              <p className="text-sm text-gray-600 mb-2">
                                ...
                                {match.parcel.sender.full_name ||
                                  match.parcel.sender.phone_number}
                              </p>
                            )}
                            {match.parcel && (
                  ...
                  <div className="text-center py-8 text-gray-500">
                    <p>
                      No matching parcels found yet. The system is actively
                      searching for parcels that match your route.
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="space-y-6">
                      ...
                      <dl className="space-y-2">
                        <div>
                          ...
                          <dd className="text-sm text-gray-900">
                            {selectedMatch.parcel.sender.full_name ||
                              "Not provided"}
                          </dd>
                        </div>
                        <div>
                  ...
                  <div className="border border-gray-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-gray-700">
                      {selectedMatch.parcel.pickup_address}
                    </p>
                  </div>
                  ...
                  <div className="border border-gray-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-gray-700">
                      {selectedMatch.parcel.delivery_address}
                    </p>
                  </div>
                    ...
                    <dl className="space-y-3">
                      ...
                      {selectedMatch.parcel.weight_kg && (
                        <div>
                          ...
                          <dd className="text-sm text-gray-900 mt-1">
                            {selectedMatch.parcel.weight_kg} kg
                          </dd>
                        </div>
                      )}
                      ...
                      <div>
                        ...
                        <dd className="text-sm text-gray-900 mt-1 capitalize">
                          {selectedMatch.parcel.status}
                        </dd>
                      </div>
                      {selectedMatch.estimated_delivery_fee != null &&
                        selectedMatch.estimated_currency && (
                          <div>
                            <dt className="text-sm font-medium text-gray-500">
                              You&apos;ll receive
                            </dt>
                            <dd className="text-sm font-semibold text-green-700 mt-1">
                              {selectedMatch.estimated_currency}{" "}
                              {Number(
                                selectedMatch.estimated_delivery_fee
                              ).toFixed(2)}
                            </dd>
                          </div>
                        )}
