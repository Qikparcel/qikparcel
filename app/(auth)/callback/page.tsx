    ...
    const handleCallback = async () => {
      ...
      try {
        ...
        if (!accessToken || !refreshToken) {
          console.error("[CALLBACK] ERROR: Missing tokens", {
        ...
        if (clientError || !clientSession) {
          console.error(
            "[CALLBACK] WARNING: Failed to set session client-side:",
            clientError
          );
        ...
        if (sessionError || !verifySession) {
          console.error(
            "[CALLBACK] ERROR: Session not accessible after setting:",
            {
              error: sessionError,
        ...
        if (!savedData) {
          console.log(
            "[CALLBACK] Form data not found, waiting 300ms and retrying..."
          );
          await new Promise((resolve) => setTimeout(resolve, 300));
        ...
        if (savedData) {
          try {
            ...
            console.log(
              "[CALLBACK] Found signup form data, updating profile...",
              {
            ...
            if (data.proofOfAddressPath) {
              updateBody.proofOfAddressPath = data.proofOfAddressPath;
            }
            if (data.selfieWithIdPath) {
              updateBody.selfieWithIdPath = data.selfieWithIdPath;
            }
            ...
            if (updateResponse.ok) {
              ...
              const errorMsg =
                updateResult.error || updateResult.message || "Unknown error";
              ...
              console.warn(
                "[CALLBACK] Continuing despite profile update failure - user can update profile later"
              );
        ...
        if (user) {
          ...
          console.log(
            "[CALLBACK] All checks passed, redirecting to",
            redirectPath
          );
          ...
          console.log(
            "[CALLBACK] User not found, redirecting to /dashboard..."
          );
          setTimeout(() => {
