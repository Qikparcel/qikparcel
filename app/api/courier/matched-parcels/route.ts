...
export async function GET(request: NextRequest) {
  try {
    ...
    const tripIdsForParcels = Array.from(
      new Set(
        parcels
          .map((p: Parcel) => p.matched_trip_id)
          .filter(Boolean) as string[]
      )
    );
    const senderIds = Array.from(
      new Set(parcels.map((p: Parcel) => p.sender_id))
    );
    ...
    const { data: tripsData, error: tripsDataError } = await adminClient
      .from("trips")
      .select(
        "id, origin_address, destination_address, departure_time, estimated_arrival, status"
      )
    ...
    const parcelIds = parcels.map((p: Parcel) => p.id);
    const { data: acceptedMatches } = await adminClient
      .from("parcel_trip_matches")
      .select("parcel_id, trip_id, payment_status")
      .in("parcel_id", parcelIds)
      .eq("status", "accepted");
    const paymentStatusByParcelId = new Map<string, Match["payment_status"]>();
    (acceptedMatches || []).forEach(
      (m: Pick<Match, "parcel_id" | "trip_id" | "payment_status">) => {
        paymentStatusByParcelId.set(m.parcel_id, m.payment_status);
      }
    );

    const processedParcels = parcels.map((parcel: Parcel) => {
      const matchedTrip = parcel.matched_trip_id
        ? tripMap.get(parcel.matched_trip_id) || null
        : null;
      ...
      return {
        ...
        sender,
        payment_status: paymentStatusByParcelId.get(parcel.id) || "pending",
      };
    });
