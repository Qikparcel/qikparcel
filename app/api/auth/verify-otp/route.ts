...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!user) {
      ...
      const tempEmail = `${formattedPhone.replace(
        /[^0-9]/g,
        ""
      )}@qikparcel.local`;
      ...
      if (createError) {
        ...
        if (
          ...
          console.log(
            "⚠️ User creation failed - email exists, searching for existing user..."
          );
          ...
          if (foundUser) {
            ...
            console.error(
              "❌ Error creating user and could not find existing user:",
              {
                error: createError,
        ...
        return NextResponse.json(
          {
            ...
            details: JSON.stringify(newUserData),
          },
          { status: 500 }
        );
      }
    ...
    if (existingProfile) {
      ...
      const profileData: Record<string, unknown> = {
        ...
        phone_number: formattedPhone,
        whatsapp_number: formattedPhone,
        role: role === "courier" ? "courier" : "sender",
        updated_at: new Date().toISOString(),
      };
      if (fullName?.trim()) profileData.full_name = fullName.trim();
      if (streetAddress?.trim())
        profileData.street_address = streetAddress.trim();
      if (addressLine2 !== undefined)
        profileData.address_line_2 = addressLine2?.trim() || null;
      if (city?.trim()) profileData.city = city.trim();
      if (state?.trim()) profileData.state = state.trim();
      if (postcode?.trim()) profileData.postcode = postcode.trim();
      ...
      console.log("Inserting profile with data:", {
        ...
        hasAddress: !!(profileData.street_address && profileData.city),
      });
      ...
      if (role === "courier" && documentPath) {
        ...
        if (kycError) console.warn("KYC insert error:", kycError);
      }
    }
