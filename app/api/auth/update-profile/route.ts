...
export async function POST(request: NextRequest) {
  try {
    ...
    // Email is optional - only add if provided (column may not exist if migration not run)
    // We'll try to update it, but if it fails, we'll catch it below
    ...
    if (!existingProfile) {
      ...
      let { data: newProfile, error: createError } = await (adminSupabase
        .from('profiles') as any)
        .insert(newProfileData)
        .select()

      // If creation failed because email column doesn't exist, retry without email
      if (createError && 'email' in newProfileData && (
        createError.message?.includes("email") || 
        createError.message?.includes("column") ||
        createError.message?.includes("schema cache")
      )) {
        console.warn('Email column may not exist, retrying profile creation without email field:', createError.message)
        ...
        if (createError) {
          ...
          console.warn('Profile created successfully without email field. Email column may not exist in database.')
        }
      }
      ...
      if (createError) {
        ...
        return NextResponse.json(
          { error: `Failed to create profile: ${createError.message}`, details: createError.details, hint: createError.hint },
          { status: 500 }
        )
      }
    ...
    if (updateError && hasEmail && (
      updateError.message?.includes("email") || 
      updateError.message?.includes("column") ||
      updateError.message?.includes("schema cache")
    )) {
      console.warn('Email column may not exist, retrying update without email field:', updateError.message)
      ...
      if (retryResult.error) {
        ...
        console.warn('Profile updated successfully without email field. Email column may not exist in database.')
