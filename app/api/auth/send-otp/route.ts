  ...
  try {
    ...
    try {
      ...
      if (!contentSid) {
        ...
        return NextResponse.json(
          { error: 'WhatsApp template not configured. Please set TWILIO_CONTENT_SID environment variable.' },
          { status: 500 }
        )
      }
    ...
    return NextResponse.json({
      ...
      ...(process.env.NODE_ENV === 'development' && { 
        otp,
        note: userExists ? 'User already exists - this is a login attempt' : 'OTP shown only in development mode'
      }),
    })
  } catch (error: any) {
