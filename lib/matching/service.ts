...
export async function findAndCreateMatchesForParcel(
  ...
  for (const trip of candidateTrips) {
    ...
    if (isMatchValid(score, MIN_SCORE_THRESHOLD)) {
      ...
      const { data: createdMatch, error: matchError } = await (
        supabase.from("parcel_trip_matches") as any
      )
        .insert(match)
        .select()
        .single();
      ...
      if (matchError) {
        ...
        console.log(
          `[MATCHING] Triggering notification for match ${createdMatch.id}`
        );
        notifyCourierOfMatch(supabase, createdMatch.id)
          .then(() => {
            console.log(
              `[MATCHING] ✅ Notification completed for match ${createdMatch.id}`
            );
          })
          .catch((error) => {
            console.error(
 ...
 */
export function calculateMatchScoreForPair(parcel: Parcel, trip: Trip): number {
